...
procedure cargar_vector(var v: vector; var cantidad_total_megusta: integer);
var
	a: foto;
	i: integer;
begin
	...
	for i:= 1 to dimF do begin
		LeerDatos(a);
		v[i]:= a;
		if(a.autor = 'Art Vandelay')then begin
...
procedure max_vistas(v: vector; var max: integer; var nombre_max: str20);
var
	i: integer;
begin
	for i:= 1 to dimF do begin
		if(v[i].cantidad_clics > max)then begin
...
Begin
	...
	maximo:= -1;
	nombre_maximo:= '';
	cargar_vector(v, cant_total_megusta);
	max_vistas(v, maximo, nombre_maximo);
